...
public partial class ModifyLevel
{
    ...
    {
        // Update existingLevelsDto with the new level information
        var existingLevelDto = existingLevelsDto.FirstOrDefault(l => l.levelNumber == level.levelNumber);
        if (existingLevelDto != null)
        {
            existingLevelDto.length = level.length;
            existingLevelDto.width = level.width;
            existingLevelDto.height = level.height;
            existingLevelDto.wallsColor = level.wallsColor;
            existingLevelDto.ceilingColor = level.ceilingColor;
            existingLevelDto.floorColor = level.floorColor;
        }
