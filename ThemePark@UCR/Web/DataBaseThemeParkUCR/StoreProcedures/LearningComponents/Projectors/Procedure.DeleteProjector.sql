CREATE PROCEDURE [DeleteProjector]
    @LearningComponentAssetId INT
AS
BEGIN
    BEGIN TRANSACTION
    IF NOT EXISTS (SELECT 1 FROM ThemePark.Projector WHERE LearningComponentAssetId = @LearningComponentAssetId)
    BEGIN
    ...
    END;
    BEGIN TRY
        DELETE FROM ThemePark.Projector
