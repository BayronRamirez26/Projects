CREATE PROCEDURE [DeleteWhiteboard]
    @LearningComponentAssetId INT
AS
BEGIN
    BEGIN TRANSACTION
    IF NOT EXISTS (SELECT 1 FROM ThemePark.Whiteboard WHERE LearningComponentAssetId = @LearningComponentAssetId)
    BEGIN
    ...
    END;
    BEGIN TRY
        DELETE FROM ThemePark.Whiteboard
