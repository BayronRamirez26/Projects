...
function ElectronicaPage() {
  const [products, setProducts] = useState([]);
  const [allArtists, setAllArtists] = useState([]);
        
  useEffect(() => {
    const fetchItems = async () => {
      const response = await fetch(`${API_BASE_URL}/products/formats/genre?q=electronic`);
    ...
    <>
      ...
      <main>
          <div className="banner">
            <img src="/categoryHeaders/genres/electronicaBanner.png" alt="Banner Electronica" className="banner-image" />
          </div>
        <CategoryLayout
          items={products}
